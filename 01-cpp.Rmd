...
:::{.hypothesis name="Homogénéité des biens"}
...
::: {.proposition name="Variations des courbes de coût"}
...
::: {.proposition name="Relations entre les coûts"}
...
L'illustration graphique est donnée sur la figure \@ref(fig:cppcout).
...
:::{.hypothesis #programmeproducteur name="Programme du producteur"}
...
On obtient donc l'équation \@ref(eq:cppopti).
:::
...
```{r profixmax, fig.cap="Maximisation du profit du producteur."}
...
(\#eq:profitCM)
\end{equation}
L'équation \@ref(eq:profitCM) nous dit que le profit d'un producteur en concurrence pure et parfaite est la différence entre le prix de vente et le coût moyen de production multiplié par le nombre d'unités vendues.
...
::: {.definition name="Demande individuelle"}
...
::: {.proposition name="Variation de la fonction de demande"}
...
::: {.remark name="Demande et demande inverse"}
...
:::{.definition name="Équilibre de marché"}
