...
L'homogénéïté des biens signifie que tous les biens vendu sur un marché sont parfaitement substituables.
...
```{r cppcout, fig.cap="Fonctions de coût"}
CT <- function(q){log(1+q)+q^2+2}
Cm <- function(q){1/(1+q)+2*q}
CM <- function(q){CT(q)/q}
CVM <- function(q){(log(1+q)+q^2)/q}
...
p <- ggplot() +
  ...
  annotate("text", x=5, y=5, label=TeX("$CVM$"), color="darkgreen") +
  annotate("text", x=5, y=7, label=TeX("$CM$"), color="red") +
  annotate("text", x=5, y=9, label=TeX("$C_m$"), color="blue")
...
2. La fonction d'offre des entreprises en concurrence pure et parfaite est égale à la fonction de coût marginal.
