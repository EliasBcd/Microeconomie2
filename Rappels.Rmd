...
```{r marche, echo=FALSE, warning=FALSE}
...
p <- ggplot(df, aes(x=x)) +
    ...
    labs(x="quantité", y = "prix", title="Equilibre de marché") +
...
```{r surplus, echo=FALSE, warning=FALSE}
...
p + geom_ribbon(data = Sc, aes(x=x, ymin=0.5, ymax = y), fill="blue") +
    ...
    labs(title = "Surplus des consommateurs producteurs")
```
...
```{r elasticite, echo=FALSE, warning=FALSE}
...
p <- ggplot(df, aes(x=x)) +
    ...
    scale_y_continuous(expand = c(0, 0), breaks = c(0.5), labels = c("p*"), limits = c(0, 1.2)) +
    scale_x_continuous(expand = c(0, 0), breaks = c(0.5), labels = c("q*"), limits = c(0, 1.2)) +
    labs(x="quantité", y = "prix", title="Perception par un producteur isolé") +
