...
Dans le cadre de ce cours, nous allons uniquement nous intéressé au pouvoir de marché des producteurs.
...
Et la condition du second ordre s'écrit :
$$
\pi''(q)<0\Leftrightarrow R''(q)-C''(q)<0\Leftrightarrow R_m'(q^*) <C_m'(q^*)
$$
:::
...
La courbe d'offre du producteur donne la quantité maximale que souhaite produire un producteur pour chaque prix.

:::

::: {#prp-courbeoffreCPP}
## Courbe d'offre en CPP
En concurrence pure et parfaite, la courbe d'offre du producteur est donnée le coût marginal.
$$p=C_m(q)$$
:::
