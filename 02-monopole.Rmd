...
On peut constater sur la figure \@ref(fig:monorecettemarginale) que deux phénomènes influencent la variation de la recette totale quand on augmente la production d'une unité (passage de $q$ à $q+\Delta q$ unités vendues).
...
```{r monorecettemarginale, fig.cap="Recette marginale d'un monopole suite à une variation de quantité."}
p1 <- p
...
::: {.proposition name="Elasticité prix de la demande et recette marginale"}
De l'équation \@ref(eq:rm), on observe que le signe de la recette marginale $R_m$ dépend de l'élasticité prix de la demande $|\varepsilon_{q/p}(q)|$.
...
R_m(q) = P(q) + qP'(q) < P(q)
\]
var $P'(q)$ est négative (la demande inverse est décroissante).
...
La figure \@ref(fig:monopoleoptimale) donne l'intuition graphique de l'égalité entre le coût marginal et la recette marginale.
...
<!-- Définir pour chaque taxe ce qu'elles font. -->
...
::: { .definition name="Économie d'échelle"}
...
::: { .remark name="Lien entre coût marginal et coût moyen en monopole naturel."} 
...
::: { .remark name="Rendements d'échelle et économies d'échelle"}
...
**Question :** Que se passe-t-il quand la fonction de demande inverse coupe la courbe de coût marginal dans la partie où le coût marginal est inférieur au coût moyen (où $C_m<C_M$) ?
...
```{r monopolediscriparfaite, fig.cap="Surplus du monopole en discrimination parfaite (aire rouge et bleue)."}
p_discriparfaite <- p
...
p_discriparfaite +   geom_function(fun=function(x) Rm(x), colour = "darkgreen") +
  ...
  annotate("text", x=0.7, y=Cm(0.7), colour="darkred", label=TeX("$C_m$")) +
...
```{r monopolediscri2degre, fig.cap="Graphique avec une discrimination du deuxième degré. Les quantités inférieures à $q_1$ sont vendues au prix $p_1$, les suivantes au prix $p_2$."}
p_discri2degre <- p
...
Sp <- data.frame(x=c(0, q1, q1, q2), ymin=c(Cm(0), Cm(q1), Cm(q1), Cm(q2)), ymax=c(p1, p1, p2, p2))
Sc <- data.frame(x=c(0, q1, q1, q2), ymax=c(P(0), P(q1), P(q1), P(q2)), ymin=c(p1, p1, p2, p2) )

p_discri2degre + geom_segment(x=0, y=p1, xend=q1, yend=p1, color='black', linetype='dashed') +
  geom_function(fun=function(x) Cm(x), colour = "red") +
  geom_segment(x=q1,y=p2, xend=q2, yend=p2, color='black', linetype='dashed') +
  geom_ribbon(data=Sp, aes(x=x, ymin=ymin, ymax=ymax), fill='red', alpha=0.5) +
  geom_ribbon(data=Sc, aes(x=x, ymin=ymin, ymax=ymax), fill='blue', alpha=0.5) +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.2), breaks = c(p1, p2), labels=c(TeX("$p_1$"), TeX("$p_2$"))) +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 1.2), breaks = c(q1, q2), labels = c(TeX("$q_1$"), TeX("$q_2$"))) +
  annotate("text", x=q1, y=p1/2, label="Surplus du monopole", color='red') +
  annotate("text", x=q1+0.1, y=p1, label="Surplus des consommateurs", color='blue') +
