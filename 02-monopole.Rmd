...
::: {.definition name="Rendements d'échelles"}
...
qm <- (143+sqrt(30241))/34
qcpp <- (47+sqrt(10657))/66
...
p_naturel + scale_x_continuous(expand = c(0, 0), limits = c(0, 10), breaks=c(qcpp, qm), labels=c(TeX("$q_2$"), TeX("$q_1$"))) +
...
::: { .definition name="Taxe forfaitaire" }
...
::: { .definition name="Impôt sur les bénéfices"}
...
::: {.proposition name="Evolution des quantités produites avec un prix maximal"}
...
Que se passe-t-il quand la fonction de demande inverse coupe la courbe de coût marginal dans la partie où le coût marginal est inférieur au coût moyen (où $C_m<C_M$) ?
...
::: {.example name="Exemple de discrimination parfaite"}
...
Tous les membres d'un groupe paient donc le même prix, contrairement à la discrimination au premier degré.
...
\end{array}
\]
L'équation \@ref(eq:cpo3e) nous dit que le monopole réparti la quantité totale produite de manière à égaliser les recettes marginales entre les deux groupes.
