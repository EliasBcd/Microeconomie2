...
```{r recette_marginale, include=T, fig.cap="Recette marginale d'un monopole."}
p1 <- p
...
*Rappel :* $\frac{P(q)}{q}\frac{dq}{dP(q)} = \varepsilon_{q/p} <0$
...
1. Si la demande est élastique ($|\varepsilon_{q/p}|>1$), alors $R_m(q)>0$, donc si la quantité augmente, le revenu augmente.
2. Si la demande est inélastique ($|\varepsilon_{q/p}|<1$), alors $R_m(q)<0$, donc si la quantité augmente, le revenu diminue.

```{r elasticite, fig.cap="Elasticité et profit."}
p2 <- p;
...
```{r profit, fig.cap="Recette maximale."}
...
1. En utilisant l'aire du trapèze vert (intégrale de la recette marginale entre 0 et la quantité produite $p^*$) ;
...
p5 +   stat_function(fun=function(x) Rm(x), colour = "darkgreen") +
  ...
  annotate("text", x=0.15, y=0.6, label=TeX("perte si $q<q_m^*$"), color="darkred") +
  annotate("text", x=0.4, y=0.4, label=TeX("perte si q>q_m^*"), color="darkred") +
...
```{r monopolerecette, fig.cap="Recette, coût et profit."}
...
```{r monopoleprofit, fig.cap="Demande, prix, coûts et profits."}
...
\[
\frac{1}{|\varepsilon_{q/p}(q^*)|} = \frac{P(q^*) - C_m(q^*)}{P(q^*)} (\#eq:Lerner)
\]
...
p_changement <- ggplot() +
  ...
  labs(x="quantité", y = "prix") +
  annotate("text", x=0.1, y=1, label=TeX("demande 2 ($R_2$)"), color="blue") +
  annotate("text", x=0.4, y=0.2, label=TeX("$R_{2m}$"), color="lightblue") +
  annotate("text", x=0.5, y=1, label=TeX("$C_m$"), color="red") +
  annotate("text", x=0.4, y=0.1, label=TeX("$R_{1m}$"), color="green") +
  annotate("text", x=0.5, y=0.4, label=TeX("demande 1 ($R_1$)"), color="darkgreen")


p_prix <- p_changement +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.2), breaks=c(5/12, 5/6), labels=c(TeX("p_1^*"), TeX("p_2^*"))) +
  scale_x_continuous(expand = c(0, 0), limits=c(0, 1.2), breaks=c(1/6), labels=c(TeX("$q_1^*=q_2^2$")))
...
p_quantité <- p_changement +
  scale_y_continuous(expand = c(0, 0), limits = c(0, 1.2), breaks=c(5/12), labels=c(TeX("p_1^*"))) +
  scale_x_continuous(expand = c(0, 0), limits=c(0, 1.2), breaks=c(1/6), labels=c(TeX("$q_1^*")))
